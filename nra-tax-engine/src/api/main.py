...
IMPORTANT: QUADTAX_API_KEY environment variable is REQUIRED at startup.
This prevents the engine from running in an unauthenticated state.
"""
...
# CRITICAL: Validate QUADTAX_API_KEY early
if "QUADTAX_API_KEY" not in os.environ:
    print("CRITICAL: Missing QUADTAX_API_KEY environment variable. Exiting.")
    sys.exit(1)
...
def healthz() -> dict:
    """Health check with real dependency validation.

    Issue #9: Validates critical dependencies to detect misconfiguration early.
    """
    checks = {
        "quaddragon_api_key": bool(os.getenv("QUADTAX_API_KEY")),
...
@app.delete(
    "/api/v1/erase-user/{user_id}",
    tags=["privacy"],
    dependencies=[Depends(require_api_key)],
)
def erase_user(user_id: str) -> dict:
    """GDPR/CCPA right-to-erasure endpoint.

    Issue #13: Deletes all state files associated with a user_id.
    Files are stored in outputs/<user_id>/ and audit logs are purged.
    """
    import shutil
    from datetime import datetime, timedelta

    user_dir = Path("outputs") / user_id
    deleted_files = []

    # Delete user's output directory
    if user_dir.exists():
        shutil.rmtree(user_dir)
        deleted_files.append(str(user_dir))

    # Purge audit logs older than 30 days for this user
    audit_dir = Path("outputs") / "audit_logs"
    if audit_dir.exists():
        cutoff = datetime.now() - timedelta(days=30)
        for log_file in audit_dir.glob(f"{user_id}_*.jsonl"):
    ...
    return {
        ...
        "message": f"User {user_id} data erased per GDPR/CCPA request."
    }
    """Serve a generated packet file. Only allows files under outputs/ directory.
